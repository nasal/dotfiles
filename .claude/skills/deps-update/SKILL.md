---
name: deps-update
description: Update dependencies safely — patch/minor in one batch, majors one at a time with changelog review, full /check after every batch, revert what breaks. Use for "update deps", "bump packages", or routine maintenance.
---
...
3. **Patch + minor — one batch**:
   ...
   2. Run `/check` (full gate: typecheck, lint, tests, build).
...
4. **Majors — one package at a time**, riskiest last:
   ...
   3. `/check`. Green → commit `upgrade <pkg> to v<X>`. Red and not trivially fixable → revert the package, report why, move on.

5. **Report**: table of updated / skipped-with-reason / needs-your-decision. Never leave the repo red — the final state must pass `/check`.
